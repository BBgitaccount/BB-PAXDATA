...
# Let's define the exact replacements for the upgrade function
...
for target, replacement in upgrade_replacements.items():
    # normalize spaces/newlines slightly to ensure exact matches
    if target in content:
        content = content.replace(target, replacement)
    else:
        # try without formatting variances
        normalized_target = target.replace("\r\n", "\n")
        content = content.replace(normalized_target, replacement)

# Now, let's define the exact replacements for the downgrade function
...
print("Migration file constraints successfully named!")
