...
from bb_paxdata.infrastructure.ai.fail_check import AIFailCheck, ValidationStatus
        ...
        for seg_idx, seg in enumerate(segments_data, 1):
            ...
            for sent_idx, sentence_text in enumerate(seg["sentences"], 1):
                ...
                await EventPublisher(session).emit(
                    ...
                    payload={
                        "sentiment_score": (
                            float(_ai_sent) if _ai_sent is not None else 0.0
                        ),
                        "sentiment_category": pipeline_res.analysis.ai_sentiment_label
                        or "",
                        "risk_score": int(_normalized_risk),
                        "hedging_score": float(_hedging_score),
                        "politeness_score": float(_politeness_ratio),
                        "logic_result": _logic_result,
                    },
                    actor_id="system",
                )
            ...
            meili_docs = [
                {
                    ...
                    "AI_Birincil_Konu": s.dominant_topic,
                    "created_at": (s.created_at or utc_now()).isoformat(),
                }
                for s in all_processed_sentences
            ]
            if meili_docs:
...
async def rebuild_network_for_file(session: Any, file_id: str) -> None:
    ...
    try:
        ...
        from bb_paxdata.infrastructure.nlp.maoz_dyadic_service import (
            MaozDyadicService,
        )
