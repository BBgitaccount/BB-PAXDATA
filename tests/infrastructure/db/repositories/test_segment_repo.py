...
def test_segment_repo_get_with_sentences_eager(db_session) -> None:
    seed_panel_speaker_segment(db_session)
    srepo = SentenceRepository(db_session)
    srepo.add(Sentence(id="sx1", text="one", speaker_id="sp1", segment_id="seg1"))
    srepo.add(Sentence(id="sx2", text="two", speaker_id="sp1", segment_id="seg1"))
    db_session.commit()

    seg_repo = SegmentRepository(db_session)
    loaded = seg_repo.get_with_sentences("seg1")
...
def test_segment_repo_temporal_analysis(db_session) -> None:
    seed_panel_speaker_segment(db_session)
    from bb_paxdata.infrastructure.db import models as m

    seg = db_session.get(m.Segment, "seg1")
    ...
    seg_repo = SegmentRepository(db_session)
    ta = seg_repo.get_temporal_analysis("seg1")
...
def test_segment_add_many(db_session) -> None:
    seed_panel_speaker_segment(db_session)
    ...
    repo.add_many(segs)
    db_session.commit()
    assert repo.get("g2") is not None
    assert repo.get("g3") is not None
